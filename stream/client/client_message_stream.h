...
public:
    ...
    MessageStream(RoleType role_type,
            IOService& io_service,
            const Endpoint& endpoint)
        : ByteStream(io_service, endpoint)
...
private:
            ...
            std::size_t bytes_transferred)
    {
        ...
        _last_rw_ticks = _ticks;  //update ticks time .
        ...
        while (!is_closed() && !received_messages.empty())
        {
            ...
            on_received(item.message, item.meta_size, item.data_size); // call last layer method...
            ...
            std::size_t bytes_transferred)
    {
        ...
        if (static_cast<int>(bytes_transferred) == _sending_size)
        {
            ...
            else
            {
                ...
                // call hook function
                on_sent(_sending_message, _sending_cookie); // last layer.
                clear_sending_env();
                ...
                && atomic_comp_swap(&_send_token, TOKEN_LOCK, TOKEN_FREE) == TOKEN_FREE)
        {
            if (get_from_pending_queue(&_sending_message, &_sending_cookie))
            {
                ...
                else
                {
                    ...
                    //SCHECK(ret);
                    async_write_some(_sending_data, _sending_size); //parent methods.
                    started = true;
                    break;
                }
            }
    ...
    int identify_message_header(char* data, int size, int* consumed)
    {
        ...
        *consumed = copy_size;
        ...
        {
            // check 1: header magic str
            if (!_receiving_header.CheckMagicString())
            {
                //SLOG(ERROR, "identify_message_header(): %s: check magic string failed: 0x%8x",
                //        EndpointToString(_remote_endpoint).c_str(), _receiving_header.magic_str_value);
                return -1;
            }
            // check 2: message_size = meta_size+data_size.
...
private:
    ...
    int64_t _received_message_size; // current received size of the message
